  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <Link to="/patients/new"><Button icon={Plus}>مريض جديد</Button></Link>
      </div>

      <div className="relative">
        <Search className="absolute right-4 top-1/2 -translate-y-1/2 w-5 h-5 text-[#A09E9B]" />
        <Input placeholder="بحث باسم المريض أو رقم الهاتف أو رقم الملف..." value={search} onChange={(e) => { setSearch(e.target.value); setPage(1); }} className="pr-12 bg-white" />
      </div>
          ...
          {data?.data?.map((patient) => (
            <Card key={patient.id} hover>
              <CardContent className="p-5 flex items-center justify-between">
                <Link to={`/patients/${patient.id}`} className="flex items-center gap-4 min-w-0 flex-1">
                  <div className="w-12 h-12 bg-[#153751]/8 rounded-2xl flex items-center justify-center shrink-0">
                    <span className="text-[#153751] font-bold text-lg">{patient.full_name?.charAt(0)}</span>
                  </div>
                  <div className="min-w-0">
                    ...
                    <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-sm text-[#7E8991] mt-1">
                      ...
                      {patient.address && <span className="flex items-center gap-1 truncate"><MapPin className="w-3 h-3" /> {patient.address}</span>}
                    </div>
                  </div>
                </Link>
                <div className="flex items-center gap-3 shrink-0 mr-4">
                  ...
                  {patient.age && <div className="text-left hidden sm:block"><span className="text-xs text-[#7E8991] block">العمر</span><span className="text-sm text-[#132D42]">{patient.age} سنة</span></div>}
                  {patient.gender && <Badge variant="muted">{patient.gender}</Badge>}
                  {patient.blood_type && <Badge variant="info">{patient.blood_type}</Badge>}
                  <Link to={`/patients/${patient.id}/edit`}><Button variant="ghost" size="sm" icon={Edit} /></Link>
                  <Link to={`/patients/${patient.id}`}><Button variant="ghost" size="sm" icon={Eye} /></Link>
                </div>
              </CardContent>
            </Card>
          ))}
