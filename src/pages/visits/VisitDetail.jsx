        ...
        <Card><CardHeader><CardTitle className="flex items-center gap-2"><Pill className="w-5 h-5" /> الوصفات</CardTitle></CardHeader><CardContent className="space-y-2">
          {visit.prescriptions.map((rx) => (
            <div key={rx.id} className="p-3 bg-[#F2EFEE] rounded-xl">
              ...
              <Button variant="ghost" size="sm" icon={Printer} onClick={() => openApiFile(`/prescriptions/${rx.id}/pdf`, `prescription-${rx.id}.pdf`).catch(() => toast.error('تعذرت طباعة الوصفة'))} className="mt-2">طباعة</Button>
            </div>
          ))}
        </CardContent></Card>
      )}
