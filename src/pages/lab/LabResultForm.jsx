...
export default function LabResultForm() {
  ...
  const { data: request } = useGetQuery(requestId ? ['lab-request', requestId] : null, requestId ? `/lab-requests/${requestId}` : null);

  // ملء البيانات تلقائياً عند اختيار الطلب
      ...
      <form onSubmit={(e) => { e.preventDefault(); createResult.mutate(form); }} className="space-y-6">
        <Card>
          ...
          <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium mb-1">طلب الفحوصات *</label>
              <SmartSelect endpoint="/lab-requests?status=مطلوب" value={form.lab_request_id} onChange={handleRequestSelect} placeholder="اختر طلب الفحوصات..." displayField="request_number" valueField="id" />
            </div>
            <div>
              <label className="block text-sm font-medium mb-1">اسم المختبر</label>
              <Input value={form.lab_name} onChange={(e) => setForm({ ...form, lab_name: e.target.value })} placeholder="مختبر الدقة..." />
            </div>
            <div>
              <label className="block text-sm font-medium mb-1">رقم تقرير المختبر</label>
              <Input value={form.lab_reference} onChange={(e) => setForm({ ...form, lab_reference: e.target.value })} />
            </div>
          </CardContent>
        </Card>
          ...
          <CardContent className="space-y-4">
            {form.results_json.map((row, i) => (
              <div key={i} className="grid grid-cols-12 gap-2 items-center">
                <div className="col-span-4"><Input placeholder="اسم الفحص" value={row.test_name} onChange={(e) => updateRow(i, 'test_name', e.target.value)} required /></div>
                ...
                <div className="col-span-1 flex justify-center">
                  <input type="checkbox" checked={row.is_abnormal} onChange={(e) => updateRow(i, 'is_abnormal', e.target.checked)} className="w-5 h-5 rounded accent-[#C85C5C]" />
                </div>
                <div className="col-span-1">
                  {form.results_json.length > 1 && <Button variant="ghost" size="icon" onClick={() => removeRow(i)} className="text-[#C85C5C]"><Trash className="w-4 h-4" /></Button>}
                </div>
              </div>
            ))}
            <Button variant="outline" icon={Plus} onClick={addRow} type="button">إضافة نتيجة</Button>
          </CardContent>
        </Card>

        <Card>
          <CardContent className="pt-6">
            <div><label className="block text-sm font-medium mb-1">ملاحظات</label><Input value={form.notes} onChange={(e) => setForm({ ...form, notes: e.target.value })} /></div>
          </CardContent>
        </Card>
